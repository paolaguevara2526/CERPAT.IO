...
const VENC_META: Record<string, { label: string; color: string }> = {
  pendiente: { label: 'Pendiente', color: 'var(--muted)' }, presentado_sin_pago: { label: 'Presentado (sin pago)', color: 'var(--info)' },
  presentado_pagado: { label: 'Presentado y pagado', color: 'var(--exito)' }, presentado_cero: { label: 'Presentado en $0', color: 'var(--cero)' },
  no_presentado: { label: 'No presentado', color: 'var(--peligro)' }, no_obligado: { label: 'No obligado', color: 'var(--neutro)' },
};
...
export default function PortalCalendario() {
  ...
  return (
    <>
      ...
      {error && <div className="panel" style={{ padding: '10px 14px', color: 'var(--peligro-fuerte)', fontWeight: 600, marginBottom: 10 }}>{error}</div>}
                ...
                {celdasVis.slice(w * cols, w * cols + cols).map((dia, idx) => {
                  ...
                  return (
                    <td key={idx} style={{ border: '1px solid var(--line)', verticalAlign: 'top', height: 92, padding: 4, background: esFestivo ? 'rgba(207,68,54,0.06)' : undefined }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: esFestivo ? 'var(--peligro)' : 'var(--muted)', marginBottom: 3, display: 'flex', alignItems: 'center', gap: 4 }}>
                        <span style={{ ...(esHoy ? { background: 'var(--navy)', color: '#fff', borderRadius: 10, padding: '0 6px' } : {}) }}>{dia}</span>
                        {esFestivo && <span style={{ fontSize: 8, fontWeight: 800, textTransform: 'uppercase', color: 'var(--peligro)' }}>Festivo</span>}
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                        {items.map((e) => (
                          <div key={e.key} title={`${e.titulo}${e.empresa ? ' · ' + e.empresa : ''} · ${e.vencido ? 'Vencido' : e.estadoLabel}`}
                            style={{ fontSize: 10, padding: '2px 5px', borderRadius: 4, background: 'var(--panel-2)', borderLeft: `3px solid ${e.vencido ? 'var(--peligro)' : e.color}` }}>
                            ...
                            {mostrarEstados && <div style={{ fontSize: 9, fontWeight: 700, color: e.vencido ? 'var(--peligro)' : e.color, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.vencido ? 'Vencido' : e.estadoLabel}</div>}
                          </div>
                        ))}
                      </div>
                    </td>
      ...
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 10, fontSize: 11.5, color: 'var(--muted)' }}>
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><span style={{ width: 10, height: 10, borderRadius: 3, background: 'var(--peligro)' }} /> Vencido</span>
