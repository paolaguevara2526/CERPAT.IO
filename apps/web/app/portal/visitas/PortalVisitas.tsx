...
import { metaModalidad } from '@/lib/modalidad';
...
type Visita = {
  id: string; empresa: string | null; fecha: string; hora: string | null; horaSalida: string | null; almuerzoMinutos: number | null; modalidad: string | null; lugar: string | null; area: string | null;
              ...
              <div key={v.id} className="panel" style={{ overflow: 'hidden' }}>
                <button onClick={() => setAbierta(open ? null : v.id)} style={{ width: '100%', textAlign: 'left', border: 'none', background: 'none', cursor: 'pointer', padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 12, fontFamily: 'var(--ui)', color: 'var(--ink)' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12.5, color: 'var(--muted)', marginTop: 2 }}>{metaModalidad(v.modalidad).icono} {metaModalidad(v.modalidad).label} · {fFecha(v.fecha)}{v.hora ? ` · ${v.hora}` : ''}{v.horaSalida ? ` a ${v.horaSalida}` : ''}{duracionTexto(v.hora, v.horaSalida, v.almuerzoMinutos) ? ` · ${duracionTexto(v.hora, v.horaSalida, v.almuerzoMinutos)}` : ''}{esFirma && v.empresa ? ` · ${v.empresa}` : ''}{v.asesor ? ` · ${v.asesor}` : ''}</div>
                  </div>
