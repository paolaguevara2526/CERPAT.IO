...
export default async function PagosPage({ searchParams }: { searchParams?: Record<string, string> }) {
  ...
  const hayFiltro = !!(estadoPago || cliente || soloVencidas || periodo !== mesActual || alcance !== 'control');
  ...
  const kpiItems = [
    ...tareasScope.map((t) => ({ estado: t.estadoPago, fecha: t.fechaVencimiento, valor: t.valorPago ?? 0 })),
    ...vencsScope.map((v) => ({ estado: v.estado, fecha: v.fechaVencimiento, valor: v.valorPago ?? 0 })),
  ];
  ...
  // Filas de las tablas: alcance + estado + "solo vencidas". Orden por urgencia
  // (no pagadas primero, la más vencida arriba).
  const filtroFila = (est: string, fecha: string) =>
    (!estadoPago || est === estadoPago) && (!soloVencidas || esVencidoSinPagar(fecha, pagadoDe(est)));
  const tareas = tareasScope.filter((t) => filtroFila(t.estadoPago, t.fechaVencimiento))
  ...
  const vencs = vencsScope.filter((v) => filtroFila(v.estado, v.fechaVencimiento))
  ...
  return (
    <>
      ...
      <p style={{ fontSize: 13, color: 'var(--muted)', margin: '0 0 14px' }}>Control de obligaciones con pago (DIAN/entidades). Digita el valor y marca el estado; el semáforo prioriza lo urgente y lo vencido.</p>
      ...
      {!error && kpiItems.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 12, marginBottom: 16 }}>
          ...
          <div className="tile" style={{ borderColor: kVencido.n > 0 ? '#e0a3a0' : undefined }}><div className="k">Vencido sin pagar</div><div className="v" style={{ color: kVencido.n > 0 ? '#d64b3f' : '#8a94a6', fontSize: 21 }}>${fmtCOP(kVencido.v)}</div><div className="s">{kVencido.n} en riesgo de sanción</div></div>
        </div>
      )}
