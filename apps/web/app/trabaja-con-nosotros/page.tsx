...
import { Header, Footer, Hero, BRAND, MUTED, NAVY, CONTACTO, card, main } from '../_sitio/Sitio';
  ...
  return (
    <main style={main}>
      ...
      <section style={{ maxWidth: 900, margin: '0 auto', padding: '54px 24px 10px', textAlign: 'center' }}>
        <h2 style={{ fontSize: 27, fontWeight: 900, margin: '0 0 12px' }}>Postúlate ahora</h2>
        <p style={{ color: MUTED, fontSize: 17, lineHeight: 1.7, margin: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 22 }}>
          {RAZONES.map((r) => (
            <div key={r.titulo} style={card}>
              <div style={{ width: 50, height: 50, borderRadius: 12, background: 'rgba(72,213,151,0.14)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, marginBottom: 14 }}>{r.icono}</div>
              <h3 style={{ fontSize: 18.5, fontWeight: 800, margin: '0 0 8px' }}>{r.titulo}</h3>
              <p style={{ color: MUTED, fontSize: 15, lineHeight: 1.65, margin: 0 }}>{r.texto}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ maxWidth: 820, margin: '0 auto', padding: '44px 24px 70px' }}>
        <div style={{ ...card, textAlign: 'center', padding: '34px 28px' }}>
          <h2 style={{ fontSize: 23, fontWeight: 900, margin: '0 0 10px' }}>Envíanos tu información</h2>
          <p style={{ color: MUTED, fontSize: 15.5, lineHeight: 1.7, margin: '0 0 22px' }}>
          ...
          <a href={`mailto:${CONTACTO.email}?subject=${ASUNTO}&body=${CUERPO}`} className="dbtn primary" style={{ fontSize: 15.5, padding: '13px 28px' }}>
            Enviar mi hoja de vida
          </a>
          <p style={{ fontSize: 13, color: '#667085', margin: '16px 0 0' }}>
            O escríbenos directamente a <a href={`mailto:${CONTACTO.email}`} style={{ color: NAVY, fontWeight: 700 }}>{CONTACTO.email}</a>
            {' · '}PBX <a href={CONTACTO.telefonoHref} style={{ color: NAVY, fontWeight: 700 }}>{CONTACTO.telefono}</a>
          </p>
        </div>
      </section>
