// apps/web/app/api/portal/[...path]/route.ts
// Proxy autenticado (catch-all) hacia los endpoints /hallazgos de la API
// (Portal de Hallazgos · Revisoría Fiscal).
...
async function forward(req: Request, path: string[], method: string) {
  ...
  const res = await fetch(`${API_URL}/hallazgos/${suffix}`, {
    method,
