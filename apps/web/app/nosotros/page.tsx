...
import { Header, Footer, Hero, Clientes, FuentePoppins, INDIGO, INDIGO_2, BRAND, LILA, TEXTO, btnVerde, btnBlanco, card, main } from '../_sitio/Sitio';
...
const EQUIPO = [
  { foto: 'paola-guevara-2', nombre: 'Paola Guevara', cargo: 'Socia Directora · Gerente General', detalle: 'Especialista en Gestión y Auditoría Tributaria, Finanzas y Revisoría' },
  { foto: 'nataly-gualtero-2', nombre: 'Nataly Gualtero', cargo: 'Socia Directora', detalle: 'Magíster en Aseguramiento Financiero y Sostenibilidad Corporativa · Especialista en Control Gerencial Corporativo' },
  { foto: 'lidis-pardo-2', nombre: 'Lidis Pardo', cargo: 'Coordinadora Administrativa y Financiera', detalle: 'Contadora Pública' },
  { foto: 'tania-castiblanco-', nombre: 'Tania Castiblanco', cargo: 'Coordinadora de Auditoría y Revisoría Fiscal', detalle: 'Contadora Pública · Especialista' },
];
  ...
  return (
    <main style={main}>
      ...
      <Hero
        ...
        imagen="/sitio/foto-apoyo-1b.webp"
      />

      {/* Quiénes somos */}
      <section style={{ maxWidth: 900, margin: '0 auto', padding: '56px 24px 10px' }}>
        <p style={{ color: TEXTO, fontSize: 17, lineHeight: 1.75, margin: '0 0 18px' }}>
        ...
        <p style={{ color: TEXTO, fontSize: 17, lineHeight: 1.75, margin: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 18 }}>
          {CIFRAS.map((c) => (
            <div key={c.etiqueta} style={{ ...card, textAlign: 'center' }}>
              <div style={{ fontSize: 38, fontWeight: 700, color: INDIGO, lineHeight: 1 }}>{c.valor}</div>
              <div style={{ color: TEXTO, fontSize: 13.5, marginTop: 8, fontWeight: 600 }}>{c.etiqueta}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ background: LILA, padding: '58px 24px', marginTop: 20 }}>
        <div style={{ maxWidth: 1120, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))', gap: 22 }}>
          <article style={card}>
            ...
            <h2 style={{ fontSize: 23, fontWeight: 700, margin: '0 0 12px' }}>Nuestra Misión</h2>
            <p style={{ color: TEXTO, fontSize: 15.5, lineHeight: 1.75, margin: 0 }}>
          ...
          <article style={card}>
            ...
            <h2 style={{ fontSize: 23, fontWeight: 700, margin: '0 0 12px' }}>Nuestra Visión</h2>
            <p style={{ color: TEXTO, fontSize: 15.5, lineHeight: 1.75, margin: 0 }}>
      ...
      <section style={{ maxWidth: 1120, margin: '0 auto', padding: '58px 24px 10px' }}>
        <div style={{ textAlign: 'center', marginBottom: 34 }}>
          ...
          <h2 style={{ fontSize: 28, fontWeight: 700, margin: 0 }}>Pasos para solicitar asesoría</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 22 }}>
          {PROCESO.map((p) => (
            <div key={p.n} style={card}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: INDIGO, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 17, marginBottom: 14 }}>{p.n}</div>
              <h3 style={{ fontSize: 18, fontWeight: 800, margin: '0 0 8px' }}>{p.titulo}</h3>
              <p style={{ color: TEXTO, fontSize: 15, lineHeight: 1.65, margin: 0 }}>{p.texto}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ maxWidth: 1120, margin: '0 auto', padding: '58px 24px 70px' }}>
        <div style={{ textAlign: 'center', marginBottom: 34 }}>
          ...
          <h2 style={{ fontSize: 28, fontWeight: 700, margin: 0 }}>Conócenos</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: 22 }}>
          {EQUIPO.map((p) => (
            <div key={p.nombre} style={{ ...card, textAlign: 'center' }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={`/sitio/${p.foto}.webp`} alt={p.nombre} style={{ width: 116, height: 116, borderRadius: '50%', objectFit: 'cover', objectPosition: 'top center', margin: '0 auto 14px', display: 'block', background: LILA }} />
              <h3 style={{ fontSize: 17.5, fontWeight: 800, margin: '0 0 4px' }}>{p.nombre}</h3>
              <div style={{ color: INDIGO, fontWeight: 600, fontSize: 13.5, marginBottom: 8 }}>{p.cargo}</div>
              <p style={{ color: TEXTO, fontSize: 13, lineHeight: 1.6, margin: 0 }}>{p.detalle}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ background: `linear-gradient(120deg, ${INDIGO}, ${INDIGO_2})`, color: '#fff', padding: '62px 24px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontSize: 29, fontWeight: 700, margin: '0 0 12px', letterSpacing: '-0.5px' }}>¿Trabajamos juntos?</h2>
          <p style={{ opacity: 0.92, fontSize: 17, margin: '0 0 26px', lineHeight: 1.7 }}>
            Cuéntanos qué necesita tu empresa y te acompañamos con un equipo especializado.
          </p>
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="/contacto" style={btnVerde}>Solicitar asesoría</a>
